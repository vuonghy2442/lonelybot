...
/-! NON-wrap sanity: i=1, j=2, len=4, step=1 — adjacent, expect EQUAL end cursors -/
...
/-! WRAP at step 3, len 3, cursor 1: order 1 (cC at 2 first) — expect the SECOND move to FAIL
    (position 0 unreachable from the saturated cursor) -/
#eval ((mkSt [cA, cB, cC] 1 3).applyDrawTo cC (Sum.inl Anchor.p0) >>= fun s => s.applyDrawTo cA (Sum.inl Anchor.p1)).isSome
/-! order 2 (cA at 0 first) — expect SUCCESS (leading lane), end cursor 1 = len-2 -/
...
/-! NON-adjacent divergence sanity: i=0, j=2, len 3, step 1 — expect cursors j-1=1 vs i=0 -/
